...
exports.add = async (req, res) => {
  ...
  try {
    ...
    if(title.length <= 25 && author.length <= 50 && email && file) { // if fields are not empty...
      ...
      if(fileExt === 'jpg' || fileExt === 'png' || fileExt === 'gif' && title === correctTitle && author === correctAuthor && email === correctEmail){
    ...
    if(!photoToUpdate) res.status(404).json({ message: 'Not found' });
      ...
      if(!voter) {
        ...
        if(voter.votes.includes(req.params.id)) {
          throw new Error ('You have already voted, you cannot vote for this photo again!');
        } else {
